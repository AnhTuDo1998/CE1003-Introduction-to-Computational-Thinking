...
This Python script contain the source code and main body for the Telegram Bot named TheForce_Bot (telegram username: ntu_cal_bot)
It is implemented using telepot api by nikoala on the following GitHub repo: https://github.com/nickoala/telepot
This is a submission for "Telegram Bot" assignment module CE1003: Introduction to Computational Thinking by team TheForce
Made in NANYANG TECHNOLOGICAL UNIVERSITY (NTU), Singapore
Information about our university: http://www.ntu.edu.sg/Pages/home.aspx

Basic Information about this project:
#Group ---------- TheForce

#Member --------- Do Anh Tu, Denzel Looi Xiongkai, Khor Zi Yun, Jesslyn Chew Sze Yu

Brief Introduction--------------------------------------------------------
NTU students are familiar with STAR wars at the beginning of every semester when they bid for modules and index groups that is good for their ideal schedule.
However, they face trouble during the aftermaths trying to remember their lessons timing, locations and frequency of occurence.
Not to mention about the painstaking process of keying the modules into Microsoft Outlook calendar of any other calendar and set reminder for attending the class themselves.
This TheForce_Bot solves that problem by receving inputs of modules and group index from users, generate and return an iCalendar file that is compatible to any existing calendar app.

This project is done with reference from Python csv-ics converter.py script by albertyw on this GitHub repo: https://github.com/albertyw/csv-to-ical
'''
...
All_flags = ["GetCal", "CourseCode_flag", "CheckCourse_flag", "ClassIndex_flag",\
             "ExtraOptions_flag", "Remove_flag", "Generate_flag", "Feedback_flag", "Semester_flag"]
...
def reset_flags(index_RS):
    global GetCal, CourseCode_flag, CheckCourse_flag, ClassIndex_flag,\
           ExtraOptions_flag, Remove_flag, Generate_flag, Feedback_flag, Semester_flag
...
def handle(msg):
    ...
    print (GetCal, CourseCode_flag, CheckCourse_flag, ClassIndex_flag,\
           ExtraOptions_flag, Remove_flag, Generate_flag, Semester_flag)
    print (ModulesName)
    print (ModulesData)
...
def on_chat_message(msg_OCM):
    ...
    if msg_OCM["text"] == "/start":
        bot.sendMessage(chat_id, """ Hello there! I am The Force (Bot), and I am here to simplify your life in NTU.
Use me, retrieve an electronic calendar for Your Modules (You will need to give me your course code and group index) or NTU Key Events, you can. 
May TheForce be with you.

To begin, click /getcal will you!
click /help for more information """)
    ...
    #Step 1: "/getcal" command for options of ICal: for Specific Courses or NTU Key Events
        ...
        GetCal.append(chat_id)
        
    #Step -1: "/reset" command to reset data and values
    elif msg_OCM["text"] == "/reset":
        bot.sendMessage(chat_id, "Reset, all data, has been. Please type /getcal if you would like to begin once again")
        reset_data(chat_id)
        reset_flags(chat_id)
        
    #Step -2: "/help" to provide more commands
    elif msg_OCM["text"] == "/help":
        bot.sendMessage(chat_id, """ Pretty easy to use, TheForce_Bot is, follow the instructions you will!
...
Type /feedback if you have any remarks on the bot""")
    ...
    #Step -3: "/feedback" for user remarks
        ...
        bot.sendMessage(chat_id, "Feedback, what on, my Padawan?")
        Feedback_flag.append(chat_id)

    elif chat_id in Feedback_flag:
        bot.sendMessage(chat_id, "Thank you, your feedback for... :)")
    ...
    #Step 3: After receiving Course Code, check whether the retrieved data is correct
        ...
        bot.sendMessage(chat_id, "Patience you must have, young Padawan...")
        CourseInput_3 = msg_OCM["text"]
        #Check for minimum length of the keyword or coursecode for the searching to be efficient
        if len(CourseInput_3) > 3:
            ...
            # the course code exist but it is an online course, user either input a different course or return to settings
                ...
                bot.sendMessage(chat_id, """Online course, this module is, thus no input for the timetable(aren't you glad).
    Please select another module:""", reply_markup=return_markup)
            ...
            # course code exist, get user confirmation that the course is correct
                ...
                bot.sendMessage(chat_id, "Looking for this course, you are? (Yes/No)", reply_markup=YesNo_markup)
                reset_flags(chat_id)
                CheckCourse_flag.append(chat_id)
        else:
            bot.sendMessage(chat_id, "Too short, your words are! Try again: ", reply_markup=return_markup)
            reset_flags(chat_id)
            Remove_flag.append(chat_id)
            CourseCode_flag.append(chat_id)
...
def on_callback_query(msg_OCQ):
    ...
    if chat_type in GetCal:
    #Step 2a: Choosing option for getting course ICal => input CourseCode
        if chat_id == "Course":
            bot.sendMessage(chat_type, "Enter your course code, you will:")
...
http://www.ntu.edu.sg/Students/Undergraduate/AcademicServices/AcademicCalendar/Pages/AY2016-17.aspx """)
        ...
        if chat_id == "Y":
            bot.sendMessage(chat_type, "Enter your class index, you will:")
            reset_flags(chat_type)
        ...
        if chat_id == "ADD":
            ...
            bot.sendMessage(chat_type, "Enter your course code/name you will:",reply_markup=return_markup)
            reset_flags(chat_type)
        ...
        #Step 6b: REMOVE modules
            ...
            bot.sendMessage(chat_type, "Which module to remove, you like?", reply_markup=markup_code)
            reset_flags(chat_type)
            Remove_flag.append(int(chat_type))
        ...
        #Step6di : Enquire on the the which semester
        elif chat_id == "CONTINUE":
            # InlineKeyboard for all SEM
            Semester_markup = InlineKeyboardMarkup (inline_keyboard=[
                [InlineKeyboardButton(text='AY17-18 SEM 1', callback_data='0')],
                [InlineKeyboardButton(text='AY17-18 SEM 2', callback_data='1')],
                [InlineKeyboardButton(text='AY18-19 SEM 1', callback_data='2')],
                [InlineKeyboardButton(text='AY18-19 SEM 2', callback_data='3')],
                [InlineKeyboardButton(text='AY19-20 SEM 1', callback_data='4')],
                [InlineKeyboardButton(text='AY19-20 SEM 2', callback_data='5')]
                ])
            bot.sendMessage(chat_type, "Which Semester would you like to generate for?", reply_markup=Semester_markup)
            reset_flags(chat_type)
            Semester_flag.append(int(chat_type))
            
    #Step6dii : Export ICal
    elif chat_type in Semester_flag:
        bot.sendMessage(chat_type, "Patience you must have, young Padawan...")
        # Obtain location of data of modules for the user 
        index_6d = UserData(chat_type)
        # Generate the ICal
        ICal_Generator(ModulesData[index_6d][1:],ModulesName[index_6d][1:],chat_type,int(chat_id))
        # Send the ICal doc (Specific to chat_id)
        bot.sendDocument(chat_type, open("calendar" + str(chat_type)+".ics"))
        bot.sendMessage(chat_type, """Here the ICal you wanted, it is! If you want to obtain a different ICal, click or type /getcal""")
...
# 4. ICAL Generator
#################################################################################################################
def ICal_Generator(finallist_Ical, finalname_Ical, chat_type_Ical, chat_id_Ical):
    Semester_Ical = ["14/8/2017","15/1/2018", "13/8/2018", "14/1/2019", "12/8/2019", "13/1/2020"]
    ...
    with open("calendar" + str(chat_type_Ical)+".csv", "w") as c:
        ...
        for Module_Ical in finallist_Ical:
            ...
            for row_Ical in Module_Ical:
                ...
                First_monday_Ical = Semester_Ical[chat_id_Ical]
                dates_list_Ical = []
                #subject located at this current row and first column
                subject_Ical = finalname_Ical[counter_Ical] + " {}".format(row_Ical[1])
                #start time located at this current row and column 5th
                starttime_Ical = row_Ical[4]
                #end time located at this current row and column 6th
                endtime_Ical = row_Ical[5]
                #all day event is set to false
                alldayevent_Ical = "False"
                #type of lession and group number is located at 2nd, 3rd, 8th column
                description_Ical = " Group: %s \n %s" %(row_Ical[2],row_Ical[7])
                #location located at this current row and 7th column
                location_Ical = row_Ical[6]
                ...
                if wk_segment_Ical == "":
                    ...
                    total_wk_Ical = []
                    # Remove the "Wk" in the string
                    wk_segment_Ical = wk_segment_Ical.replace("Wk","")
